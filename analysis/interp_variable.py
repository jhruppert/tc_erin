    ...
    for i in variable_name:
        ...
        if i == 'V':
            ...
            if levels == 1:
                ...
                output_dataset = nc.Dataset(output_dir + input_file[-3:] + 'interp_V', 'w', clobber=True)
            output_dataset.setncatts(dataset.__dict__)
            # Create the dimensions based on global dimensions, with exception to bottom_top
            for dim_name, dim in dataset.dimensions.items():
                if dim_name == 'bottom_top':    output_dataset.createDimension(dim_name, levels)
...
input_file = "/ourdisk/hpc/radclouds/auto_archive_notyet/tape_2copies/tc_erin/stitchd02_wrfout.nc"
variable_name = 'U'
variable_name2 = 'V'
#vertical_levels = np.arange(1000,700,-100)
vertical_levels = np.array([500, 1000, 3000])
output_dir = "/ourdisk/hpc/radclouds/auto_archive_notyet/tape_2copies/colin/erin/test/output/postproc/"
print("Levels: ", vertical_levels)
interp_variable(input_file, variable_name, output_dir, vertical_levels)
interp_variable(input_file, variable_name2, output_dir, vertical_levels)
